...
//
bool POLYCONF_acf_has_generated_header(const POLYCONF::NODE& root)
{
...
//
void POLYCONF_acf_write_generated_header(std::ostringstream& stream, const POLYCONF::NODE& root)
{
    if (!POLYCONF_acf_has_generated_header(root))
    {
        POLYCONF_acf_write_comment_line(stream, GENERATED_COMMENT);
    }
}
...
std::string POLYCONF::write_acf(const POLYCONF::CONFIG& config)
{
    ...
    POLYCONF_acf_write_generated_header(stream, config.root());
